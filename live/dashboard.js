...
function updateEdgeLivePnL() {
  // Keep the Screen-4 "vs S&P 500" headline tick-synced with Screen 1's hero —
  // identical math (stats.realized + TP1 banked + unrealized, over MEXC equity).
  ...
  const tp1Banked = enriched.reduce((s, t) => s + (t.tp1BankedUsd || 0), 0);
  const mexcUnreal = state.mexcAccount?.unrealized_pnl;
  const unrealized = (mexcUnreal != null) ? mexcUnreal : enriched.reduce((s, t) => s + (t.usd || 0), 0);
  const total = (stats.realized_pnl_usd || 0) + tp1Banked + unrealized;
  const equity = (state.mexcAccount?.equity) || (b && b.base_usd) || 1660;
  ...
  const setPct = (id, v) => {
    ...
    el.className = "an-stat-v " + _signCls(v);
  };
...
function renderEdgeScreen() {
  ...
  // "Ours" computed EXACTLY like Screen 1's hero so the % reconciles:
  //   total = stats.realized + TP1 banked + unrealized;  return = total / MEXC equity.
  const openTrades = (state.trades || []).filter(t => t.status === "OPEN");
  const enriched = openTrades.map(t => ({ ...t, ...computeUnrealized(t) }));
  const tp1Banked = enriched.reduce((s, t) => s + (t.tp1BankedUsd || 0), 0);
  const mexcUnreal = state.mexcAccount?.unrealized_pnl;
  ...
  const equity = (state.mexcAccount?.equity) || (a.benchmark && a.benchmark.base_usd) || 1660;
...
function _renderBenchmark(b, liveTotalPnl, equity) {
  ...
  const base = equity || b.base_usd || 1660;
  const oursNowPct = base ? (liveTotalPnl / base) * 100 : 0;
  const scale = (b.base_usd && base) ? (b.base_usd / base) : 1;   // realized curve /base_usd → /equity

  const toMs = iso => new Date((iso || "").length <= 10 ? iso + "T00:00:00Z" : iso).getTime();
  ...
  // Index-based even spacing (same as Screen 1's equity curve) — x is strictly
  // increasing so the smoothed line can never loop/reverse. Both series span the
  // full width: left = inception, right = now.
  ...
  // Legend swatches: Spyker follows performance colour; S&P blue.
...
// Default collapsed; state persisted per browser.
(function wireHkCollapse(){
  const KEY = "hkCollapsed_live_v1";
  function apply(){
